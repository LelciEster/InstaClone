...
<div class="min-h-screen bg-black text-white flex">
    ...
    <aside class="hidden md:flex flex-col w-56 border-r border-neutral-800 px-4 py-8 sticky top-0 h-screen shrink-0">
      ...
      <nav class="flex flex-col gap-1 flex-1">
        ...
        {/if}
                 ...
                 <!-- Settings -->
      <a href="/settings" class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors {active('/settings') ? 'bg-neutral-800 text-white font-semibold' : 'text-neutral-400 hover:text-white hover:bg-neutral-900'}">
        <svg class="w-6 h-6 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
      ...
      <form method="POST" action="/logout">
        <button type="submit" class="flex items-center gap-3 px-3 py-2.5 w-full rounded-xl text-neutral-500 hover:text-white hover:bg-neutral-900 transition-colors">
          <svg class="w-6 h-6 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  <!-- ── Mobile Top Bar (nur wenn eingeloggt) ── -->
  {#if user}
    <div class="md:hidden fixed top-0 left-0 right-0 z-50 bg-black border-b border-neutral-800 h-12 flex items-center justify-between px-4">
      <span class="text-lg font-black tracking-tighter" style="font-family:'Georgia',serif">momento</span>
      <div class="flex items-center gap-4">
        ...
        <a href="/upload" class="text-neutral-400 hover:text-white">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/>
          </svg>
        </a>
        <a href="/profile/{user.username}" class="text-neutral-400 hover:text-white">
          <div class="w-6 h-6 rounded-full bg-neutral-700 overflow-hidden flex items-center justify-center">
            {#if user.avatar_url}
              <img src={user.avatar_url} alt={user.username} class="w-full h-full object-cover"/>
            {:else}
              <span class="text-[9px] font-bold text-white">{user.username[0].toUpperCase()}</span>
            {/if}
          </div>
        </a>
      </div>
  ...
  <!-- ── Main content ── -->
  <main class="flex-1 {user ? 'md:pt-0 pt-12' : ''}">
    {@render children()}
  </main>
